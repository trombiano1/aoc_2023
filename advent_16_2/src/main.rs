...
fn main() {
    ...
    for i in 0..map.len() {
        ...
        dfs(i as isize, 0, 0, &mut seen, &map);
        let mut now = 0;
        for l in &seen {
            for c in l {
                if c[0] || c[1] || c[2] || c[3] {
    ...
    for j in 0..map[0].len() {
        let mut seen = vec![vec![vec![false; 4]; map[0].len()]; map.len()];
        dfs(0, j as isize, 1, &mut seen, &map);
        let mut now = 0;
        for l in &seen {
            for c in l {
                if c[0] || c[1] || c[2] || c[3] {
    ...
    for i in 0..map.len() {
        let mut seen = vec![vec![vec![false; 4]; map[0].len()]; map.len()];
        dfs(i as isize, (map[0].len() - 1) as isize, 2, &mut seen, &map);
        let mut now = 0;
        for l in &seen {
            for c in l {
                if c[0] || c[1] || c[2] || c[3] {
    ...
    for j in 0..map[0].len() {
        let mut seen = vec![vec![vec![false; 4]; map[0].len()]; map.len()];
        dfs((map.len() - 1) as isize, j as isize, 3, &mut seen, &map);
        let mut now = 0;
        for l in &seen {
            for c in l {
                if c[0] || c[1] || c[2] || c[3] {
